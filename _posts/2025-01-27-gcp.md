...
# Architecture for Batch Job failure update in Google Chat
The work is mainly by Pulumi, Python and GCP service, will mainly demonstrate the complete workflow here.
1. Create Pub/sub topic and subscription, for the filter in subscription, add below filter
    ```
    # CASE-SENSITIVE
    filter: attributes.NewJobState="FAILED"
    ```
2. Create cloud run service witrh the correct endpoint, in this case, I use `/message` endpoint, push to container registry and save it as variable image_uri in pulumi config file of cloud run service
3. In subscription pulumi.yaml, set push config as below
    ```
    pushConfig:
        pushEndpoint: ${cloudrun_service}/message
        oidcToken:
            serviceAccountEmail: ${builder}
    ```

    For cloud run service URL, you can fetch dynamically by below
    ```
    cloud_run_service_name: xxx
    ...
    cloudrun_service:
        ...
        return uri
    ```
4. In batch Job pulumi file, add below lines in create batch job resource step so that the status update can be pushed to the topic
    ```
    notifications:
        pubsubTopic: projects/${project}/topics/${pubsub_topic}
        message:
